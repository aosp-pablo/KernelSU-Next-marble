...
if grep -q '"custom"' config/managers.json; then
  fail "legacy custom manager choices remain selectable"
fi
...
for name, entry in managers.items():
    if entry["repo"] == "pershoot/KernelSU-Next":
        raise SystemExit(f"FAIL: forked manager source is selectable main repo for {name}")

ksun = managers["kernelsu-next"]
susfs = ksun.get("susfs")
if not isinstance(susfs, dict):
    raise SystemExit("FAIL: kernelsu-next must have susfs config")
if susfs.get("repo") != "pershoot/KernelSU-Next":
    raise SystemExit("FAIL: KernelSU-Next SUSFS repo must be pershoot/KernelSU-Next")
if susfs.get("ref") != "dev-susfs":
    raise SystemExit("FAIL: KernelSU-Next SUSFS ref must be dev-susfs")

ksu = managers["kernelsu"]
if ksu.get("susfs") is not False:
    raise SystemExit("FAIL: kernelsu must have susfs: false")
PY
