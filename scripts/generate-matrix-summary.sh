...

{
  ...
  if [[ -n "${source_repo}" ]]; then
    echo "| Kernel source | [\`${kernel_source_id}\`](https://github.com/${source_repo}) |"
  else
    echo "| Kernel source | \`${kernel_source_id}\` |"
  fi
  ...
  if [[ "${enable_susfs_first}" == "true" && -n "${susfs_display}" ]]; then
    echo "| SUSFS | \`${susfs_display}\` · \`${susfs_branch}\` · [\`$(short_commit "${susfs_commit}")\`](${susfs_url}) |"
  else
    echo "| SUSFS | Disabled |"
  fi
  ...
  echo "|:---|:---|"
  if [[ -n "${source_repo}" ]]; then
    echo "| Kernel source | [\`${source_repo}\`](https://github.com/${source_repo}) |"
  else
    echo "| Kernel source | ${kernel_source_id} |"
  fi
